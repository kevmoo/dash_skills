...
Each skill is located in `skills/<skill-name>/` and must contain a `SKILL.md`
file.
...
- **Be Concise**: Agents have limited context windows. Focus on information the
  agent doesn't already have.
- **Imperative Tone**: Use imperative or infinitive form for instructions (e.g.,
  "Use `isA<T>()` for type checks" rather than "You should use...").
...
- **Tool Integration**: If a skill uses scripts in `scripts/`, explain how the
  agent should invoke them.
...
- **Keep Descriptions Accurate**: The frontmatter `description` is the only
  thing the agent reads _before_ activating the skill. Ensure it clearly states
  the "when to use" scenarios.
- **Test Scripts**: Ensure all scripts in `scripts/` are executable and produce
  LLM-friendly output.
...
4.  (Optional) Package the skill using `package_skill.cjs` if distribution as a
